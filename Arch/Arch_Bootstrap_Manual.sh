...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/RaZ0rr-Two/dotfiles.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/RaZ0rr-Two/InitArch/master/progs.csv"
...
adduserandpass() { \
	...
	useradd -m -G wheel,audio,video "$name" >/dev/null 2>&1 ||
	usermod -a -G wheel,audio,video "$name" && mkdir -p /home/"$name" && chown "$name":"$name" /home/"$name"
...
for x in curl git wget python-pip linux-headers linux-lts-headers ca-certificates base-devel zsh ; do
...
manualinstall paru || error "Failed to install AUR helper(paru)."
...
putgitrepo "$dotfilesrepo" "/home/$name/gFolder/RaZ0rr" "$repobranch"
...
%wheel ALL=(ALL) NOPASSWD: /usr/bin/shutdown,/usr/bin/reboot,/usr/bin/systemctl suspend,/usr/bin/wifi-menu,/usr/bin/mount,/usr/bin/umount,/usr/bin/pacman -Syu,/usr/bin/pacman -Syu --noconfirm,/usr/bin/pacman -Syyu,/usr/bin/packer -Syu,/usr/bin/packer -Syyu,/usr/bin/systemctl restart NetworkManager,/usr/bin/rc-service NetworkManager restart,/usr/bin/pacman -Syyu --noconfirm,/usr/bin/loadkeys,/usr/bin/paru,/usr/bin/pacman -Syyuw --noconfirm"
